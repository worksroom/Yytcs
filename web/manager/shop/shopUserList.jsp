    ...
    <script type="text/javascript">
        ...
        $(function () {
            grid = $("#maingrid").ligerGrid({
                checkbox: true,
                columns: [
                    {display: '主键', name: 'id', align: 'left', width: 50, hide: true},
                    {display: '店铺ID', name: 'shopId', align: 'left', width: 100, hide: true},
                    {display: '店员ID', name: 'userId', minWidth: 100},
                    {display: '店员昵称', name: 'nickName', minWidth: 150},
                    {display: '类型', name: 'type', minWidth: 100, render: function (item) {
                        var txt = "未知"
                        if(item.type==1){
                            txt = "店铺创建人"
                        } else if(item.type==2){
                ...
                dataAction: 'server',
                url: 'manager/shop.do?method=findShopUids',
                usePager: false,
                pageSize: 20,
                rownumbers: true,
                width: '100%',
        ...
        function addData(){
            $.ligerDialog.open({
                url: 'manager/shop/shopUserAdd.jsp', height: 400, width: 300, buttons: [
                    {
                        ...
                        $.ajax({
                            cache: true,
                            type: "POST",
                            url:"manager/shop.do?method=addShopUser",
                            data: formData,
                            async: false,
                            error: function(data) {
                                $.ligerDialog.tip({
